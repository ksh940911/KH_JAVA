...
/**
 * 데이터모델을 만들거고, 그안에 데이터가 저장됨
 * JTable(TableModel)
 * 		- TableModel 데이터가 저장되는 객체, 이벤트핸들링
 * JScrollPane(Jtable)을 추가
 * 		- 헤더부 노출(표 각각 부분의 항목), 스크롤기능을 이용하기 위한 목적
 *
 */
...
//		};
		...
		model.addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				...
				//꼭 변경되지 않아도, 포커스를 가졌다가 잃을때, 콘솔창에 행,열 정보(몇열 몇행이 바뀌었는지)가 출력됨
				int row = e.getFirstRow();
				int col = e.getColumn();
//				System.out.println(row + ", " + col);
				//실제 값의 변경을 알아보기
				//꼭 변경되지 않아도, 포커스를 가졌다가 잃을때, 콘솔창에 바뀐 데이터가 출력됨
